...
export const hasId = (obj: { 0: number | null; 1: number | null; 2: number | null; 3: number | null; 4: number | null }, id: number) => {
	return getKeys(obj).reduce((prev, curr) => prev || obj[curr] === id, false);
};

export const isMuted = (flagss: Flag, ids: SelectedIds, hero: Hero) => {
	let flagRole = 0;
	let flagMark = 0;
